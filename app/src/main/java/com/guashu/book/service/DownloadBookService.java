...
import com.guashu.book.api.support.Logger;
import com.guashu.book.api.support.LoggingInterceptor;
...
public class DownloadBookService extends Service {
    ...
    public synchronized void downloadBook(final DownloadQueue downloadQueue) {
        AsyncTask<Integer, Integer, Integer> downloadTask = new AsyncTask<Integer, Integer, Integer>() {
            ...
            BookSource source = downloadQueue.bookSource;
                ...
                for (int i = start; i <= end && i <= list.size(); i++) {
                    ...
                    if (!downloadQueue.isFinish && !downloadQueue.isCancel) {
                        ...
                        if (CacheManager.getInstance().getChapterFile(bookId, i) == null) {
                            ...
                            int ret = download(source, url, bookId, chapters.title, i, list.size());
                            if (ret != 1) {
                                failureCount++;
                            }
                        } else {
    ...
    private int download(BookSource source, String url, final String bookId, final String title, final int chapter, final int chapterSize) {

        final int[] result = {-1};
        IBookApi bookApi = BookApiManager.getInstance().getAllBookApi().get(source);
        Subscription subscription = bookApi.getChapter(url)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<ChapterRead.Chapter>() {
                    @Override
                    public void onNext(ChapterRead.Chapter data) {
                        if (data != null) {
                            post(new DownloadProgress(bookId, String.format(
