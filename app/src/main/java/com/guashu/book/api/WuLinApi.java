 ...
 * @author user
 */
public class WuLinApi implements IBookApi {
    private static final BookSource BOOK_SOURCE = BookSource.WULIN;
    ...
    public List<SearchDetail.SearchBooks> searchBooKFuzzy(String name) {
        ...
        String url = "http://zhannei.baidu.com/cse/search?q=" + name
                + "&s=13049992925692302651&entry=1";
        try {
    ...
    public ChapterRead.Chapter getChapterRead(String chapterUrl) {
        try {
            Document document = Jsoup.connect(
                    chapterUrl).get();
            String body = document.select(".contentbox.clear").text();
