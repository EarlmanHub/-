...
public class LocalPageFactory implements IPageFactory {
    ...
    public LocalPageFactory(Context context, String bookId, List<BookMixAToc.mixToc.Chapters> chaptersList) {
        this(context, ScreenUtils.getScreenWidth(), ScreenUtils.getScreenHeight(),
                SettingManager.getInstance().getReadFontSize(),
                bookId, chaptersList);
    }

    public LocalPageFactory(Context context, int width, int height, int fontSize, String bookId,
...
// 保存阅读进度
            SettingManager.getInstance().saveReadProgress(bookId, currentChapter, curBeginPos, curEndPos, 1);
        }
    }
...
// 保存当前页的值
    ...
    public int openBook(int chapter, int[] position, int currentPage) {
        return openBook(chapter, position);
    }
