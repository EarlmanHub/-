...
public abstract class BaseReadView extends View {
    ...
    protected IPageFactory pagefactory = null;
    ...
    public BaseReadView(Context context, boolean isLocal, String bookId, List<BookMixAToc.mixToc.Chapters> chaptersList,
        ...
        if (isLocal) {
            pagefactory = new LocalPageFactory(getContext(), bookId, chaptersList);
        } else {
    ...
    public synchronized void init(int theme) {
        if (!isPrepared) {
            try {
                ...
                int ret = pagefactory.openBook(pos[0], new int[]{pos[1], pos[2]}, pos[3]);
                if (ret == 0) {
    ...
    public boolean onTouchEvent(MotionEvent e) {
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (cancel) {
                    pagefactory.resetCurrentPage(isAdd);
                    pagefactory.cancelPage();
                    restoreAnimation();
                    postInvalidate();
                } else {
    ...
    public void jumpToChapter(int chapter) {
        resetTouchPoint();
        pagefactory.openBook(chapter, new int[]{0, 0}, 1);
        pagefactory.onDraw(mCurrentPageCanvas);
        pagefactory.onDraw(mNextPageCanvas);
        postInvalidate();
    }
    ...
    public void setPosition(int[] pos) {
        int ret = pagefactory.openBook(pos[0], new int[]{pos[1], pos[2]}, 1);
        if (ret == 0) {
            listener.onLoadChapterFailure(pos[0]);
            return;
        }
